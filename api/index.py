...
def api_recommend():
    ...
    if not openai_client:
        ...
        elif "돈가스" in lunch or "고기" in lunch:
            return jsonify({
                "analysis": "점심에 든든한 고기 요리를 먹었으니, 저녁은 소화가 잘 되는 가벼운 채소 위주의 식단이 좋겠어요.",
                "recipes": [{"name": "아삭 콩나물 무침", "desc": "깔끔하고 고소한 맛의 국민 반찬", "time": "10", "diff": "매우 쉬움", "ingredients": ["콩나물", "다진마늘", "참기름", "소금"], "steps": ["콩나물을 데친 후 찬물에 헹군다", "준비한 양념장에 조물조물 무친다"], "tip": "너무 오래 데치지 않아야 아삭함이 살아요"}],
                "message": "기름진 점심 뒤에 깔끔한 채소 반찬으로 속을 편안하게 해주는 센스! 오늘도 건강한 한 끼를 준비하는 당신을 응원합니다. 🌿"
            })
        else:
            return jsonify({
